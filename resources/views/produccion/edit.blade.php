...
@section('content')
    <div class="relative bg-white">
        ...
        <div class="p-6 space-y-6">
            <form action="{{ route('produccion.update', $item->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-6 gap-6">
                    ...
                    <div class="col-span-6 sm:col-span-3">
                        ...
                        <input type="text" name="referencia" id="referencia"
                            class="shadow-sm bg-gray-100 border border-gray-300 text-gray-400 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                            value="{{ $item->referencia }}"  readonly>
                    </div>
                    <div class="col-span-6 sm:col-span-3">
                        <label for="gestor_id" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Gestor</label>
                        <select name="gestor_id" id="gestor_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            <option value="" disabled>Selecciona un administrador</option>
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}" @if ($item->user->id == $user->id) selected @endif>{{ $user->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-span-6 sm:col-span-3">
                        <label for="cliente" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Cliente
                            de la producción</label>
                        <select name="cliente" id="cliente"
                            class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                            <option value="" disabled selected>Selecciona un cliente</option>
                            @foreach ($clientes as $cliente)
                                <option value="{{ $cliente->id }}" @if ($item->cliente_id == $cliente->id) selected @endif>
                                    {{ $cliente->nombre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-span-6 sm:col-span-3">
                        ...
                        <select name="estado" id="estado" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            @foreach ($estados as $estado)
                                <option value="{{ $estado }}" @if ($item->estado == $estado) selected @endif>{{ $estado }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-span-6">
                        ...
                        <textarea id="descripcion" name="descripcion" rows="4"
                            ...
                            placeholder="Breve descripción del procedimiento a realizar">{{ $item->descripcion }}</textarea>
                    </div>
